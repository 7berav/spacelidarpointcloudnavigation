...
def generate_full_hexagon_prism(N):
    # 기존 함수와 똑같이 전체 6면을 고르게 생성하도록 확장
    # 지금은 일부 면만 생성되니, 좌우 대칭 복사라도 먼저 해도 충분함
    base = generate_random_points_on_hexagon_prism(N)
    mirrored = base.copy()
    mirrored[:, 0] *= -1
    full = np.vstack([base, mirrored])
    return full[:N]
...
def regression_polynomial(points, symb_terms):
    ...
    # zeros_vec = np.zeros((points.shape[0], 1))
    # beta = np.linalg.lstsq(A, zeros_vec, rcond=None)[0].flatten()
    # error = zeros_vec.flatten() - np.dot(A, beta)
    return beta, error
...
def visualize_implicit_surface(f_expr, center_shift=None, level=1.0, grid_size=80, grid_range=6):
    ...
    F = f_func(X, Y, Z)
    print("min:", np.min(F), "max:", np.max(F))
...
condition = PPm_body[:, 2] - 1.5 * PPm_body[:, 0] > -0.3
PPm_body = PPm_body[condition]
...
q0 = [np.cos(17 / 57.92), np.sin(17 / 57.92), 0, 0]
q_scipy = np.roll(q0, -1)
rotm = R.from_quat(q_scipy).as_matrix()
PPmR_body = np.dot(PPm_body.copy(), rotm.T)

PPm_use = PPm_body.copy()

PPm_use = PPm_use - np.mean(PPm_use, axis=0)
PPm_use += np.random.normal(scale=1e-3, size=PPm_use.shape)
...
print("beta min:", np.min(beta))
print("beta max:", np.max(beta))
...
verts_3, faces_3 = visualize_implicit_surface(f_rotated_final)
